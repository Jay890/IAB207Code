...
class DestinationForm(FlaskForm):
    ...
    description = TextAreaField('Description', validators=[InputRequired('Description is required'),
                                                           Length(min=10, max=300,
                                                                  message='Description is too short or too long')])
...
class RegisterForm(FlaskForm):
    ...
    confirm_password = PasswordField('Enter same password', validators=[InputRequired('Password is required'),
                                                                        EqualTo('password',
...
class CommentForm(FlaskForm):
    comment = TextAreaField('Comment', validators=[InputRequired("Please add comment"),
                                                   Length(min=5, max=100, message='Comment is too long or too short')])
